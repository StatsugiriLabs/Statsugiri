...
from utils.constants import FORMAT_ID_EVENT_ARG
...
def lambda_handler(event: LambdaDict, context: LambdaContext) -> dict:
    ...
    """
    replay_snapshot_format = event.get(FORMAT_ID_EVENT_ARG)
    ...
    team_snapshot = ps_replay_transform_client.transform(event)
